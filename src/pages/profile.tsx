...
const Profile: React.FC = () => {
  ...
  return (
    <div id='ProfilePage' className='bg-gray-400 w-full min-h-screen'>
      <div id="navbar" className='bg-indigo-950 w-full h-16 flex items-center text-white fixed z-50 top-0 left-0'>
        ...
        <div id="ProfileCard" className='flex flex-col items-center justify-center bg-white p-10 m-10 w-2/3 rounded-lg shadow-lg mt-20'>
          <p className='text-3xl p-10'>Profile</p>
          <div className='flex flex-col items-start w-full'>
            <p className='text-xl'>Username: {user?.username}</p>
            <p className='text-xl'>Email: {user?.email}</p>
            <p className='text-xl'>Full Name: {user?.name}</p>
          </div>
        </div>
    </div>
  )
